...
export default function TravelIntelligence({ ident, date, origin, destination, departure, refreshed, cached, onRefresh, hideConnection, externalConnection, flightDeparture, flightArrival }) {
  ...
  return <section className="travel-intelligence" aria-labelledby="travel-intelligence-title">
    <header><div><span className="traveler-kicker">At your airports</span><h3 id="travel-intelligence-title">Weather &amp; travel updates</h3></div><button aria-label="Refresh additional travel information" onClick={()=>setAttempt(v=>v+1)} disabled={state.loading}><RefreshCw size={17}/></button></header>
    ...
    {data && <>
      <details className="source-check-details"><summary>{comparison?.conflicts?.length?'Sources disagree — check with your airline':'Where these updates come from'}</summary><div className={`provider-check ${comparison?.conflicts?.length?'has-conflict':''}`}>
      ...
      <div className="weather-details"><div className="airport-outlooks">{data.airports?.map(a=><article key={a.side} className={a.forecast?.status==='available'?'forecast-available':'forecast-missing'}>
