...
export default function CarrierLogo({flight}){
 ...
 const name=flight.operator||iata;
 ...
 return <div className="carrier-logo">{iata&&stage!=='text'?<img src={url} alt={`${name} airline logo`} loading="lazy" onError={()=>setFailed({name,stage:stage==='play'?'airline':'text'})}/>:<span>{iata||<Plane size={20}/>}</span>}</div>;
}
