...
import {filterFlights} from './travel-polish.js';
...
export default function FlightSearch({go}){
 ...
 const [filters,setFilters]=useState({carrier:'',window:'all',sort:'departure'});
 useEffect(()=>setFilters({carrier:'',window:'all',sort:'departure'}),[state.options]);
 const carriers=[...new Set((state.options||[]).map(f=>flightOptionIdentity(f).carrier).filter(Boolean))];
 const visibleOptions=filterFlights(state.options||[],filters);
 ...
 return <section className="flight-finder natural-search" aria-label="Find your flight">
  ...
  {!q&&recent.length>0&&<div className="finder-examples recent-flight-chips"><span>Recently viewed</span>{recent.map(item=><button key={item.key} onClick={()=>{edit(`${item.ident} ${airportCode(item.origin)}-${airportCode(item.destination)}`);setDate(item.date);setDateOverride(true);}}>{recentFlightLabel(item)} · {new Date(`${item.date}T12:00:00Z`).toLocaleDateString([],{month:'short',day:'numeric',timeZone:'UTC'})}</button>)}<button onClick={()=>{try{localStorage.removeItem(RECENT_KEY);setRecent([]);}catch{setState({error:'This browser couldn’t clear your recent searches. Please try again.'});}}}>Clear recent</button></div>}
  {(state.options||state.error)&&<section className="finder-results" aria-live="polite"><h2 ref={resultHeading} tabIndex={-1}>{state.options?.length?'Which one is yours?':'Let’s narrow it down.'}</h2>{state.error&&<p role="alert">{state.error}</p>}{state.options?.length>0&&<><p>Match the airports and departure time to your booking. Times are local to the departure airport unless marked UTC.</p>{state.options.length>1&&<div className="flight-filter"><label>Airline<select value={filters.carrier} onChange={e=>setFilters({...filters,carrier:e.target.value})}><option value="">All airlines</option>{carriers.map(c=><option key={c} value={c}>{airlines.find(a=>a.code===c)?.name||c}</option>)}</select></label><label>Departure time<select value={filters.window} onChange={e=>setFilters({...filters,window:e.target.value})}><option value="all">Any time</option><option value="morning">Before noon</option><option value="afternoon">Noon – 6 pm</option><option value="evening">After 6 pm</option></select></label><label>Sort by<select value={filters.sort} onChange={e=>setFilters({...filters,sort:e.target.value})}><option value="departure">Earliest departure</option><option value="arrival">Earliest arrival</option></select></label><button type="button" onClick={()=>setFilters({carrier:'',window:'all',sort:'departure'})}>Reset</button></div>}{visibleOptions.length===0&&<p>No flights match these filters. Try “Any time” or “All airlines”.</p>}{visibleOptions.map((flight,i)=><FlightOption key={`${flight.ident||ident}-${i}`} flight={flight} fallback={ident} preferred={parsed.airline} onSelect={display=>open(flight,state.payload,state.routeMode?display:ident)}/>)}<small className="finder-source">{state.source}{state.partial?' · More schedules may exist; these are the results returned within this search limit.':''}{state.note?` · ${state.note}`:''}</small></>}<button className="finder-edit" onClick={()=>{reset();document.querySelector('#flight-query')?.focus();}}><ArrowLeft size={16}/>Edit search</button></section>}
