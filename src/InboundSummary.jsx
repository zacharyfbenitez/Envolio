...
import {travelerAirport} from './flight-summary.js';
...
export default function InboundSummary({flight,position,current,refreshed,cached,rotation}){
 ...
 return <section className={`inbound-summary ${info.tone}`} aria-labelledby="inbound-heading">
  ...
  {flight?.origin&&<p className="inbound-origin"><MapPin size={16} aria-hidden="true"/><span>From <strong>{travelerAirport(flight.origin)}</strong>{flight.origin.code_iata&&(flight.origin.city||flight.origin.name)&&<span className="inbound-origin-code"> · {flight.origin.code_iata}</span>}</span></p>}
