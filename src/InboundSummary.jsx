...
export default function InboundSummary({flight,position,current,refreshed,cached,rotation}){
 ...
 return <section className={`inbound-summary ${info.tone}`} aria-labelledby="inbound-heading">
  ...
  <p>{info.advice}</p>
  {flight?.scheduled_in&&<p>Scheduled gate arrival: {time(flight.scheduled_in,current.origin?.timezone)}. The arrival label compares this with the incoming flight’s latest gate time—not your departure time.</p>}
