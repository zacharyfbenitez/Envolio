...
export default function WebShell({children,go}){
 const [online,setOnline]=useState(navigator.onLine),[dockHidden,setDockHidden]=useState(false),base=import.meta.env.BASE_URL;
 useEffect(()=>{
 ...
 useEffect(()=>{
  ...
  const update=()=>{frame=0;const current=scrollY;if(current<40)setDockHidden(false);else if(current>previous+8)setDockHidden(true);else if(current<previous-8)setDockHidden(false);previous=current;};
  const scroll=()=>{if(!frame)frame=requestAnimationFrame(update);};
  addEventListener('scroll',scroll,{passive:true});
  return()=>{removeEventListener('scroll',scroll);if(frame)cancelAnimationFrame(frame);};
 },[]);
 ...
 return <div className="pwa-layout">
  ...
  <div className="dock-shelf" data-hidden={dockHidden||undefined}><nav className="pwa-dock" aria-label="Website navigation">
