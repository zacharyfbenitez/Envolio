...
test('home planes stay outside content throughout their flight paths', {timeout:180000},async t=>{
 ...
 const page=await browser.newPage(),errors=[];page.on('pageerror',e=>errors.push(e.message));await page.setRequestInterception(true);page.on('request',r=>new URL(r.url()).hostname==='127.0.0.1'?r.continue():r.abort());
 for(const width of [1366,1440,1920,390]){
  ...
  await page.goto(`http://127.0.0.1:${server.address().port}`,{waitUntil:'domcontentloaded'});await page.waitForSelector('.finder-form');
  assert.equal(await page.$$eval('.sky-plane',es=>es.length),8);
  if(width<1300){assert.equal(await page.$eval('.home-sky',e=>getComputedStyle(e).display),'none');continue;}
  for(const fraction of [0,.125,.25,.375,.5,.625,.75,.875]){
   const overlaps=await page.evaluate(f=>{
    ...
    return [...document.querySelectorAll('.sky-plane')].filter(e=>{const a=e.getBoundingClientRect();return protectedRects.some(b=>a.left<b.right&&a.right>b.left&&a.top<b.bottom&&a.bottom>b.top);}).length;
   },fraction);
   assert.equal(overlaps,0,`No overlap at ${width}px, phase ${fraction}`);
  }
  await page.screenshot({path:`/tmp/envolio-home-planes-${width}.png`});
