...
test('regional marketing numbers require an explicit provider codeshare',()=>{
 ...
 assert.equal(flightOptionIdentity({ident:'SKW1234',codeshares:['UA123','DL456']}).display,'UA123');
 assert.equal(flightOptionIdentity({ident_iata:'YX4397',codeshares:['AA4397','AS4227','AT5115']}).display,'AA4397');
...
test('status colors never turn schedules or missing data into on-time claims',()=>{
 ...
 assert.equal(flightOptionStatus({}).label,'Status unavailable');
