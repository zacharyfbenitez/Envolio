...
test('configured accounts require sign-in to Watch, preserve local saves, and explain email delivery availability',{timeout:180000},async t=>{
 ...
 let authAvailable=false,rows=[];const network=[];p.on('requestfailed',r=>network.push({failed:r.url(),reason:r.failure()?.errorText}));p.on('console',m=>{if(m.type()==='error')network.push({console:m.text()});});
